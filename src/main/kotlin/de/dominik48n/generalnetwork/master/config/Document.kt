package de.dominik48n.generalnetwork.master.config
...
class Document(private val jsonObject: JsonObject) {
    ...
    companion object {
        fun read(input: String): Document {
            return Document(MasterServer.INSTANCE.gson.fromJson(input, JsonObject::class.java))
        }

        fun read(file: File): Document? {
            if (!file.exists()) return null
            val bufferedReader = BufferedReader(FileReader(file))
            val jsonObject = MasterServer.INSTANCE.gson.fromJson(bufferedReader, JsonObject::class.java)
        ...
        fun create(file: File): Document {
            try {
                val inputStream = ClassLoader.getSystemResourceAsStream("${file.parent}\\${file.name}");
    ...
    fun write(file: File) {
        ...
        bufferedWriter.write(MasterServer.INSTANCE.gson.toJson(this.jsonObject))
